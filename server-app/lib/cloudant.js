...
const cloudant_id = process.env.CLOUDANT_ID || '484826d5-c043-4f31-ba3b-40bb41154180-bluemix'
const cloudant_apikey = process.env.CLOUDANT_IAM_APIKEY || '1Okh2q8AKb_5zBbLs3dZ-2JHe3V7Q3TZguvPoIbemjLQ';
 ...
 */
function find(type, partialName, id) {
    return new Promise((resolve, reject) => {
        let selector = {}
        if (type) {
            selector['type'] = type;
 ...
 */
function create(type, name, description, latitude, longitude, maxLimit, currentStrength) {
    return new Promise((resolve, reject) => {
        let itemId = uuidv4();
 ...
 * when the call to the DB completes
 */
function update(id, totalGuests) {
...
/*               if (type) {item["type"] = type} else {item["type"] = document.type};
                ...
                if (latitude) {item["latitude"] = latitude} else {item["latitude"] = document.latitude};
                if (longitude) {item["longitude"] = longitude} else {item["longitude"] = document.longitude};
 */
                item["type"] = document.type;
                item["name"] = document.name;
                item["description"] = document.description;
                item["latitude"] = document.latitude;
                item["longitude"] = document.longitude;
                item["maxLimit"] = document.maxLimit;
                item["currentStrength"] = parseInt(document.currentStrength) + totalGuests;
                item["whenCreated"] = document.whenCreated;
